...
function GridCiudades() {
  ...
  const limpiar = (e) => {
    ...
    document.getElementById("idEstado2").value = 0;
  };
  ...
  const peticionPost=async()=>{
    ...
    await axios.post(baseurl+"CatalogoCiudades/AgregarCiudades", consolaSeleccionada)
    .then(response=>{
      peticionGet();
      y();
      MySwal.fire({
  ...
  const peticionPut = async () => {
    ...
    await axios
      .put(baseurl+"CatalogoCiudades/ActualizarCiudades/"+ consolaSeleccionada.idCiudad, consolaSeleccionada)
      .then((response) => {
        var dataNueva = datax;
        dataNueva.map((consola) => {
          if (consolaSeleccionada.idCiudad === consola.idCiudad) {
            consola.nombre = consolaSeleccionada.nombre;
            consola.idRegion = consolaSeleccionada.idRegion;
            consola.idEstado = consolaSeleccionada.idEstado;
  ...
  const peticionDelete = async () => {
    await axios.delete(baseurl+"CatalogoCiudades/borrarCiudades/" + consolaSeleccionada.idCiudad).then((response) => {
      setData(datax.filter((consola) => consola.idCiudad !== consolaSeleccionada.idCiudad));
      MySwal.fire({
      ...
      <section className="section">
                ...
                <div className="page-hero d-flex align-items-center justify-content-center">
                  <table className="table datatable">
                    ...
                    <tbody>
                      {datax.map((consola) => (
                        <tr>
                          <th scope="row">{consola.idCiudad}</th>
                          <td>{consola.nombre}</td>
                          <td>{consola.idEstado}</td>
        ...
        <div className="modal fade" id="registroRegiones" tabIndex={-1}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Agregar Ciudad</h5>
                <button
                  type="button"
                  id="closeButton2"
                  className="btn-close"
